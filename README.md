...
Use of the repository and installation instructions and can be found below and in the respective folders.
...
The protocol uses a standardized compartment system based on the EndoA adjacency matrix (from `ListOfCompartments_sept2024.xlsx`):
...
Transport reactions are only created between adjacent compartments as defined in the EndoA matrix (e.g., m↔i for mitochondria-IMS, c↔m for cytosol-mitochondria).
...
2. generate_database: generates a metabolic network based using current available information from a large number of molecular databases:
	...
	- **Rhea Database Integration** (NEW):
		- `rhea_extension.py`: Extends the KEGG-based model with Rhea database reactions
		- `transport_utils.py`: Transport reaction compartmentalization using EndoA adjacency matrix
		...
		- Transport reactions automatically assigned to adjacent compartments (e.g., m↔i, c↔m)
	- **Standalone Rhea Model**:
		- `extend_model_with_rhea.py`: Create a Rhea-only model without KEGG base
