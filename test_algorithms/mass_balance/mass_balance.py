...
for x in cobra_model.reactions:    
	eq = re.sub(r'(^| )[0-9\.]+','',re.sub(r'e-[0-9]+','', re.sub('<=>','->', re.sub('-->','->', re.sub('  ',' ', re.sub(r'[0-9\.]+e-[0-9]+','', x.reaction)))))).strip() # correct 
	Rxn = re.sub(r'[a-z]+[0-9]*','', eq) # correct
	eq = re.sub('<=>', '->', re.sub('-->','->', re.sub(r'e-[0-9]+','', re.sub(r'(^| )[0-9\.]+','', x.reaction).strip()).strip() ) )   
	Rxn = re.sub(r'[a-z]+[0-9]*','', re.sub(r'e-[0-9]+','', re.sub(r'(^| )[0-9\.]+','', x.reaction).strip()).strip() )         
