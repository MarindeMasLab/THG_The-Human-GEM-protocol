...
def RxnCompare(eq1, eq2):
    SpeciesInEq1 = (
        [x for x in [x.replace(" ", "") for x in eq1.split("->")][0].split("+")],
        [x for x in [x.replace(" ", "") for x in eq1.split("->")][1].split("+")],
    )
    SpeciesInEq2 = (
        [x for x in [x.replace(" ", "") for x in eq2.split("->")][0].split("+")],
        [x for x in [x.replace(" ", "") for x in eq2.split("->")][1].split("+")],
    )
...
def WrapRxnSubsProdParam(Reaction, MetList, MetEquiv):
    ...
    gly_test = min(
        [
            1 if x in list(MetEquiv.keys()) else 0
...
def UnwrapRxnSubsProdParam(Reaction, LibIni, IthRxnMB):
    ...
    for x in SubsLibEnd:
        if x in LibIni[0]:
            LibIni[0][x][0] = SubsLibEnd[
                x
            ]  # Change a default St Coef (1) by the calculated one
        else:  # Metabolite in the new list but not in the original =  addedd metabolite
            if x == "H":
                LibIni[0].update(
                    {x: [SubsLibEnd[x], "C00080"]}
                )  # This line has to be replaced by a function that gives the real ID
            elif x == "H2O":
                LibIni[0].update(
                    {x: [SubsLibEnd[x], "C00001"]}
                )  # This line has to be replaced by a function that gives the real ID
            elif x == "Fe":
                LibIni[0].update(
                    {x: [SubsLibEnd[x], "C00023"]}
                )  # This line has to be replaced by a function that gives the real ID
            elif x == "Na":
                LibIni[0].update(
                    {x: [SubsLibEnd[x], "C01330"]}
                )  # This line has to be replaced by a function that gives the real ID
            elif x == "Ca":
                LibIni[0].update(
                    {x: [SubsLibEnd[x], "C00076"]}
                )  # This line has to be replaced by a function that gives the real ID
            elif x == "K":
                LibIni[0].update(
                    {x: [SubsLibEnd[x], "C00238"]}
                )  # This line has to be replaced by a function that gives the real ID
            elif x == "F":
                LibIni[0].update(
                    {x: [SubsLibEnd[x], "C00023"]}
                ...
                LibIni[0].update(
                    {x: [SubsLibEnd[x], "C00000"]}
                )  # This line has to be replaced by a function that gives the real ID
            elif x == "X":
                LibIni[0].update(
                    {x: [SubsLibEnd[x], "C0000X"]}
                )  # This line has to be replaced by a function that gives the real ID
    ProdLibEnd = dict(x for x in list(zip(IthRxnMB[7], IthRxnMB[1])))
    for x in ProdLibEnd:
        if x in LibIni[1]:
            LibIni[1][x][0] = ProdLibEnd[
                x
            ]  # Change a default St Coef (1) by the calculated one
        else:  # Metabolite in the new list but not in the original =  addedd metabolite
            if x == "H":
                LibIni[1].update(
                    {x: [ProdLibEnd[x], "C00080"]}
                )  # This line has to be replaced by a function that gives the real ID
            elif x == "H2O":
                LibIni[1].update(
                    {x: [ProdLibEnd[x], "C00001"]}
                )  # This line has to be replaced by a function that gives the real ID
            elif x == "Fe":
                LibIni[1].update(
                    {x: [ProdLibEnd[x], "C00023"]}
                )  # This line has to be replaced by a function that gives the real ID
            elif x == "Na":
                LibIni[1].update(
                    {x: [ProdLibEnd[x], "C01330"]}
                )  # This line has to be replaced by a function that gives the real ID
            elif x == "Ca":
                LibIni[1].update(
                    {x: [ProdLibEnd[x], "C00076"]}
                )  # This line has to be replaced by a function that gives the real ID
            elif x == "K":
                LibIni[1].update(
                    {x: [ProdLibEnd[x], "C00238"]}
                )  # This line has to be replaced by a function that gives the real ID
            elif x == "F":
                LibIni[1].update(
                    {x: [ProdLibEnd[x], "C00023"]}
...
def CountAtom(eq, AddH, H2O, RxnID):
    eq = (
        " " + eq
    )  # add an extra space at the beginning to avoid problems when defining StCoeff of the first metabolite using reduce func
    Ls = list("abcdefghijklmnopqrstuvwxyz")
    # Mass Balance
    Ss, Es, i, ii = defaultdict(list), [], 1, defaultdict(list)
    for p in eq.split("->"):
        for kk in p.split("+"):
            ...
            for e, m in re.findall("([A-Z][a-z]?)([0-9]*)", kk):
                m = (
                    StCoeff if m == "" else int(m) * StCoeff
                )  # Stoichiometric Coeff improvement
                d = [c[0], c[1] * m * i]
                Ss[e][:0], Es[:0] = [d], [[e, d]]
                ii[e].append(d[1])
    ...
    if MassBalance == 0:
        Species = [x.strip() for x in [x for x in eq.split("->")][0].split("+")], [
            x.strip() for x in [x for x in eq.split("->")][1].split("+")
        ]
        i = 0
        for x in Species[0]:
            if re.findall("[A-Z]", x[0]):
                Species[0][i] = "1 " + x
            else:
                Species[0][i] = x
            i += 1
    ...
    if k and not BadKTest and TestI == 0:
        ...
        if TestSol == 0:
            ...
            for x in range(len(parameters)):
                hh[parameters[x]] = Solution[x]
            Solution = [hh[x] for x in parameters_sorted]
...
def MB_REM(eq, AddH, H2O, RxnID):
    ...
    BadSolTest_a = [
        1 if i == 0 else 0 for i in Solution.tolist()
    ]  # Any stoichimetric coefficient = 0?
    BadSolTest_b = [
        1 if not str(Solution.tolist()).isdigit() else 0 for i in Solution.tolist()
    ]  # any stoichimetric coeficient is dependent?
    ...
    if not Solution.all() or BadSolTest != 0:
        ...
        if k:
            ...
            TestI = len(
                set(re.findall("[a-z]", str(list(k.values()))))
            )  # Test if more than 1 independent variable
        else:
            I = ""
            TestI = 0
...
def MB_LP(eq, AddH, H2O, RxnID):
    ...
    for x in range(0, len(C_subs)):
        locals()["bound%s" % x] = (1, 100)
        y += 1
    for x in range(y, y + len(C_prod)):
        locals()["bound%s" % x] = (-100, -1)
        y += 1
    d = locals()
    ...
    if not res.success or str(res) == "nan":
        ...
        Solution = np.round(abs(res.x) / min(abs(res.x)), decimals=10)
        LS = len(eq.split("->")[0].split("+"))
...
def mass_balance(eq, RxnID):
    ...
    if len(re.split(r"\+|->", eq)) < 20:
        ...
        MB = ListOfFunc[2](
            eq, AddH, AddH2O, RxnID
        )  # Check if the reaction is already Mass Balanced
        if MB[0]:
            TestOfBalance = 1
        else:
            TestOfBalance = 0
        while not MB[0] and i < len(ListOfFunc) - 1:
            ...
            if not MB[0] and not re.findall(
                "H[^A-Z0-9]|[^A-Z0-9]H$|^[H][^A-Z0-9]|H1[^A-Z0-9]|[^A-Z0-9]H1$|^[H1][^A-Z0-9]",
                eq,
            ):  # not H in the original equatio
                # Add H to substrates
                AddH = (
                    -1
                )  # In case it is necessary to add H+, and H+ compound has not been previously added into the model, this flag is used to include it
                AddH2O = 0  # In case it is necessary to add H2O, and H2O compound has not been previously added into the model, this flag is used to include it
                ...
                if not MB[0]:
                    ...
                    if MB[0]:
                        eq = eq.replace("->", "-> H +")
            if not MB[0] and not re.findall(
                "H2O[^A-Z0-9]|[^A-Z0-9]H2O$|^[H2O][^A-Z0-9]|H2O1[^A-Z0-9]|[^A-Z0-9]H2O1$|^[H2O1][^A-Z0-9]",
                eq,
            ):  # not H in the original equatio
                ...
                AddH2O = (
                    -1
                )  # In case it is necessary to add H2O, and H2O compound has not been previously added into the model, this flag is used to include it
                # Add H2O to products
                ...
                if not MB[0]:
                    ...
                    MB = ListOfFunc[i](
                        eq.replace("->", "-> H2O +"), AddH, AddH2O, RxnID
                    )
                    if MB[0]:
                        eq = eq.replace("->", "-> H2O +")
            if (
                ...
                MB = ListOfFunc[i](
                    eq.replace("->", "+ H2O + H ->"), AddH, AddH2O, RxnID
                )
                if MB[0]:
                    eq = eq.replace("->", "+ H2O + H ->")
                if not MB[0]:
                    ...
                    if not MB[0]:
                        ...
                        AddH2O = 1  # In case it is necessary to add H2O, and H2O compound has not been previously added into the model, this flag is used to include it
                        MB = ListOfFunc[i](
                            eq.replace("->", "+ H -> H2O +"), AddH, AddH2O, RxnID
                        )
                        if MB[0]:
                            eq = eq.replace("->", "+ H -> H2O +")
                        if not MB[0]:
        ...
        if not MB[0]:
            ...
            MEtInEq = [
                x.split(" ")[-1]
                for x in [x.strip() for x in [x for x in eq.split("->")][0].split("+")]
            ], [
                x.split(" ")[-1]
                for x in [x.strip() for x in [x for x in eq.split("->")][1].split("+")]
            ]
            IsH = (
                [1 for x in MEtInEq[0] if x == "H" or x == "H1"],
                [1 for x in MEtInEq[1] if x == "H" or x == "H1"],
            )
            IsH2O = (
                [1 for x in MEtInEq[0] if x == "H2O" or x == "H2O1"],
                [1 for x in MEtInEq[1] if x == "H2O" or x == "H2O1"],
            )
            ...
            if not IsH[0] and not IsH2O[0]:
                MB6 = [
                    ListOfFunc[i](eq.replace("->", "+ H2O + H ->"), -1, -1, RxnID),
                    -1,
                    -1,
                ]
            ...
            if not IsH[0] and not IsH2O[1]:
                MB8 = [
                    ListOfFunc[i](eq.replace("->", "+ H -> H2O +"), -1, 1, RxnID),
                    -1,
                    1,
                ]
    ...
    if not MB[0]:
        MB = (
            [1 for x in [x for x in eq.split("->")][0].split("+")],
            [1 for x in [x for x in eq.split("->")][1].split("+")],
        )
        AddH = 0  # In case it is necessary to add H+, and H+ compound has not been previously added into the model, this flag is used to include it
        AddH2O = 0  # In case it is necessary to add H2O, and H2O compound has not been previously added into the model, this flag is used to include it
        AllSpecies = (
        ...
        NewSpecies = ["", ""]
        TestOfBalance = 2
    if min(MB[0] + MB[1]) > 100:
        StchS = [x / min(MB[0] + MB[1]) for x in MB[0]]
        StchP = [x / min(MB[0] + MB[1]) for x in MB[1]]
    else:
        StchS = MB[0]
        StchP = MB[1]
    return (
...
def add_extra_compound(new_compound, lib, time, EF, specialCompounds):
    ...
    nc = compound(
        "http://www.kegg.jp/dbget-bin/www_bget?cpd:C00080",
        "C00080",
        time,
        EF,
        specialCompounds,
