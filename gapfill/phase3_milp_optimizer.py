...
def find_blocked_reactions_fast(model, zero_cutoff=None):
    ...
    try:
        ...
        return find_blocked_reactions(model, [r.id for r in model.reactions], zero_cutoff)
...
def test_candidate_coverage(model_json_path, candidate, blocked_rxns, zero_cutoff=None):
    ...
    met1 = candidate['met1']; met2 = candidate['met2']
...
def compute_coverage_matrix(model, candidates, blocked_rxns, model_json_path=None, 
    ...
    if verbose:
        print(f"Computing coverage matrix for {total} candidates over {len(blocked_rxns)} blocked reactions...")
    ...
    if verbose:
        ...
        print(f"Coverage computed: {non_empty}/{total} candidates can unblock at least one reaction")
        print(f"Total (candidate, blocked_rxn) pairs covered: {total_coverage}")
    
    return coverage


def solve_coverage_milp(coverage, blocked_rxns, candidates, 
                        max_ptrs=None, min_unblocked=None, 
                        tradeoff_lambda=0.01, verbose=True):
    """
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                raise ImportError("No MILP solver available. Install gurobipy, scipy, or pulp.")
    ...
    if verbose:
        print(f"Setting up MILP: {n_cands} candidate vars, {n_blocked} blocked-rxn vars")
    
    if solver == 'gurobi':
        return _solve_gurobi(coverage, blocked_list, blocked_idx, candidates, cand_idx,
                            max_ptrs, min_unblocked, tradeoff_lambda, verbose)
    elif solver == 'pulp':
        return _solve_pulp(coverage, blocked_list, blocked_idx, candidates, cand_idx,
                          max_ptrs, min_unblocked, tradeoff_lambda, verbose)
    else:
        return _solve_scipy(coverage, blocked_list, blocked_idx, candidates, cand_idx,
                           max_ptrs, min_unblocked, tradeoff_lambda, verbose)


def _solve_gurobi(coverage, blocked_list, blocked_idx, candidates, cand_idx,
                  max_ptrs, min_unblocked, tradeoff_lambda, verbose):
    """Solve using Gurobi."""
    import gurobipy as gp
    from gurobipy import GRB
    ...
    for b_id, b_i in blocked_idx.items():
        ...
        if covering_cands:
            m.addConstr(z[b_i] <= gp.quicksum(y[p] for p in covering_cands), name=f"cov_{b_i}")
        else:
            m.addConstr(z[b_i] == 0, name=f"nocov_{b_i}")
    
    # Optional constraints
    if max_ptrs is not None:
        m.addConstr(gp.quicksum(y[p] for p in range(n_cands)) <= max_ptrs, name="max_ptrs")
    
    if min_unblocked is not None:
        m.addConstr(gp.quicksum(z[b] for b in range(n_blocked)) >= min_unblocked, name="min_unblocked")
    ...
    if verbose:
        ...
        print(f"MILP solution: {len(selected)} PTRs selected, {n_unblocked} reactions unblocked")
    
    return selected


def _solve_pulp(coverage, blocked_list, blocked_idx, candidates, cand_idx,
                max_ptrs, min_unblocked, tradeoff_lambda, verbose):
    """Solve using PuLP."""
    import pulp
    ...
    if prob.status != pulp.LpStatusOptimal:
        if verbose:
            print(f"MILP did not find optimal solution. Status: {pulp.LpStatus[prob.status]}")
        return []
...
def _solve_scipy(coverage, blocked_list, blocked_idx, candidates, cand_idx,
                 max_ptrs, min_unblocked, tradeoff_lambda, verbose):
...
def run_phase3(candidates_csv,
    ...
    if starting_model_json is None:
        starting_model_json = os.path.normpath(os.path.join(
            os.path.dirname(__file__), '..', 'models', 'base', 
            'THG-beta-batch_251106_phase2_minimal_connected.json'))
    
    model = load_json_model(starting_model_json)
    if verbose:
        print(f"Loaded model with {len(model.reactions)} reactions, {len(model.metabolites)} metabolites")
    ...
    while iteration < max_iterations and blocked_rxns and remaining_candidates:
        ...
        if verbose:
            print(f"\n=== Iteration {iteration} ===")
            print(f"Blocked reactions: {len(blocked_rxns)}, Remaining candidates: {len(remaining_candidates)}")
        ...
        coverage = compute_coverage_matrix(model, remaining_candidates, blocked_rxns,
                                          model_json_path=temp_model_path, verbose=verbose)
        ...
        effective_candidates = [c for c in remaining_candidates 
                               if coverage.get((c['met1'], c['met2']), set())]
        
        if not effective_candidates:
            if verbose:
                print("No candidates can unblock any remaining blocked reactions. Stopping.")
            break
        ...
        # Solve MILP
        selected = solve_coverage_milp(coverage, blocked_rxns, effective_candidates,
                                       max_ptrs=max_ptrs, min_unblocked=min_unblocked,
                                       tradeoff_lambda=tradeoff_lambda, verbose=verbose)
...
if __name__ == '__main__':
    import argparse
    parser = argparse.ArgumentParser(description='MILP-based Phase-3 blocked reaction optimizer')
    ...
    if not os.path.exists(cand_csv):
        ...
        print(f'Phase3 MILP selected {n} connectors, wrote {out_csv} and model {model_out}')
