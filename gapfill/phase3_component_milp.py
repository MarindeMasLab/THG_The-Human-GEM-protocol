...
def compute_coverage_for_component(model, candidates, blocked_rxns, verbose=True):
    ...
    if verbose:
        print(f"    Computing coverage: {total} candidates × {n_blocked} blocked reactions")
    ...
    for i, cand in enumerate(candidates):
        if verbose and (i + 1) % 20 == 0:
            ...
            print(f"      Progress: {i+1}/{total} ({rate:.1f} cand/sec, ETA: {eta:.0f}s)")
    ...
    if verbose:
        ...
        print(f"    Coverage done in {elapsed:.1f}s: {non_empty}/{total} candidates useful, {total_pairs} total unblocks")
    
    return coverage


def solve_component_milp(coverage, blocked_rxns, candidates, tradeoff_lambda=0.01, verbose=True):
    """
...
def run_phase3_component_wise(
    ...
    for comp_idx, (comp_id, comp_size) in enumerate(isolated_comps):
        ...
        comp_candidates = get_candidates_for_component(candidates, comp_id, used_candidates)
        if verbose:
        ...
        coverage = compute_coverage_for_component(model, comp_candidates, blocked_in_comp, verbose)
        
        # Filter to candidates with any coverage
        effective_candidates = [c for c in comp_candidates 
        ...
        selected = solve_component_milp(coverage, blocked_in_comp, effective_candidates,
                                        tradeoff_lambda, verbose)
        ...
        if verbose:
            ...
            print(f"  After adding PTRs: {len(new_blocked)} still blocked (was {len(blocked_in_comp)})")
    ...
    with open(results_csv, 'w', newline='') as f:
        fieldnames = ['component_id', 'component_size', 'reactions_in_comp', 
                     'blocked_before', 'candidates_available', 'ptrs_selected']
...
if __name__ == '__main__':
    import argparse
    parser = argparse.ArgumentParser(description='Component-wise MILP Phase-3 optimizer')
