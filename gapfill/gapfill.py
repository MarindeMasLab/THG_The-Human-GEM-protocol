...
Usage examples:
  ...
  # Full pipeline - deadends objective
    ...
    python3 gapfill/gapfill.py run-all --pipeline blocked --phase3-mode sink_milp --components 4,5,6,7 --solver gurobi --model <unconnected_model.json>

    # Option 4: sink_milp_original - Legacy sink MILP implementation (with model export)
    python3 gapfill/gapfill.py run-all --pipeline blocked --phase3-mode sink_milp_original --components 4,5,6,7 --solver gurobi --model <unconnected_model.json> \
...
def main():
    ...
    pall.add_argument(
        "--pipeline",
        choices=["deadends", "blocked"],
        default="deadends",
        help="Objective pipeline",
    )
    pall.add_argument(
        "--candidates", help="Phase1 candidates CSV (skip phase1 if exists)"
    )
    pall.add_argument(
        "--model", required=True, help="Unconnected model JSON (phase1 input)"
    )
    ...
    pall.add_argument(
        "--force-phase1",
        action="store_true",
        help="Force re-run of phase1 even if candidates CSV exists",
    )
    pall.add_argument(
        "--phase2-mode",
        choices=["minimal", "prioritized"],
        default="minimal",
        help="Phase2 connector strategy",
    )
    pall.add_argument(
        "--phase3-mode",
        choices=[
    ...
    pall.add_argument(
        ...
        type=int,
        default=500,
        help="Top-K candidates to prefilter for hybrid strategies",
    )
    pall.add_argument(
        "--batch-size",
        type=int,
        default=50,
        help="Batch size for hybrid_batch strategy",
    )
    pall.add_argument(
        "--lambda",
        ...
        type=float,
        default=0.01,
        help="MILP tradeoff weight (higher = fewer PTRs)",
    )
    pall.add_argument(
        "--max-iter", type=int, default=10, help="Max iterations for MILP strategy"
    )
    pall.add_argument(
        "--min-comp-size",
        type=int,
        default=4,
        help="Minimum component size for component_milp strategy",
    )
    pall.add_argument(
        "--max-components",
        type=int,
        default=None,
        help="Max number of components to process (component_milp)",
    )
    ...
    pall.add_argument(
        ...
        type=str,
        default=None,
        help='Comma-separated component IDs for sink_milp (e.g., "4,5,6,7")',
    )
    pall.add_argument(
        "--parallel-components",
        action="store_true",
        help="Enable parallel processing of small components (sink_milp)",
    )
    pall.add_argument(
        "--workers-components",
        type=int,
        default=4,
        help="Workers for parallel component processing (sink_milp)",
    )
    pall.add_argument(
        "--parallel-fba",
        action="store_true",
        help="Enable parallel FBA testing within components (sink_milp)",
    )
    pall.add_argument(
        "--workers-fba",
        type=int,
        default=2,
        help="Workers for parallel FBA testing (sink_milp)",
    )
    pall.add_argument(
        "--small-threshold",
        type=int,
    ...
    pall.add_argument(
        "--phase3-model",
        dest="phase3_model_out",
        default=None,
        help="Output path for Phase-3 model (only for sink_milp_original)",
    )
    pall.add_argument(
        "--solver",
        choices=["gurobi", "cplex", "glpk", "dynamic"],
        default="dynamic",
        help="MILP solver for optimization (sink_milp). gurobi/cplex require licenses. dynamic auto-selects best available.",
    )
    pall.add_argument(
        "--solver-lp",
        choices=["glpk", "glpk_exact", "scipy", "gurobi", "cplex", "hybrid", "highs"],
    ...
    if args.cmd == "phase1":
        ...
        if not os.path.exists(model):
            print("Phase1 model not found:", model)
            sys.exit(1)
        from cobra.io import load_json_model as _load

        mc = _load(model)
        out = args.out
        cand, dead, comp = phase1_connect_components.generate_phase1_outputs(
            mc, out_dir=out
        )
    ...
    if args.cmd == "phase2":
        ...
        if args.mode == "minimal":
            ...
            out_csv, model_out, n = run_phase2(
                cand, unconnected_model_json=args.model, out_dir=args.out
            )
    ...
    if args.cmd == "run-all":
        ...
        if args.phase2_mode == "minimal":
            ...
            out_csv2, phase2_model, n2 = phase2_min.run_phase2(
                cand, unconnected_model_json=model, out_dir=out_dir
            )
            print(f"Phase-2 minimal: selected {n2} connectors -> model {phase2_model}")
        else:
            print("Running Phase-2 prioritized connector selection...")
            phase2_prior = importlib.import_module("phase2_prioritized_connector")
            out_csv2, phase2_model, n2 = phase2_prior.run_phase2(
                cand, unconnected_model_json=model, out_dir=out_dir
            )
            print(
                f"Phase-2 prioritized: selected {n2} connectors -> model {phase2_model}"
            )
        ...
        if args.pipeline == "deadends":
            ...
            ph3_dead = importlib.import_module("phase3_greedy_optimizer")
            out_csv3, phase3_model, n3 = ph3_dead.run_phase3(
                cand,
            ...
            if phase3_mode == "tiered_milp":
                ...
                out_csv3, phase3_model, n3 = ph3_tiered.run_phase3_tiered(
                    cand,
                    starting_model_json=phase2_model,
                ...
                print(
                    f"Phase-3 tiered MILP: added {n3} connectors -> model {phase3_model}"
                )
                ...
                print(
                    f"Phase-3 component MILP: added {n3} connectors -> model {phase3_model}"
                )
                ...
                print(
                    "Running Phase-3 (Sink MILP: temp sinks + original components + hybrid parallelization)..."
                )
                ...
                n3 = result.get("summary", {}).get(
                    "total_ptrs", len(result.get("all_selected", []))
                )
                ...
                print(
                    "Running Phase-3 (Sink MILP original implementation with model export)..."
                )
                ...
                print(
                    f'Phase-3 sink MILP original: selected {n3} PTRs -> {out_csv3}, model {phase3_model or "<not written>"}'
                )
            else:
                print("Running Phase-3 (blocked-reaction minimizer)...")
                ph3_block = importlib.import_module("phase3_blocked_optimizer")
                out_csv3, phase3_model, n3 = ph3_block.run_phase3(
                    cand,
                    starting_model_json=phase2_model,
                    out_dir=out_dir,
                    max_additions=args.max,
                    strategy=phase3_mode,
                    topk=args.topk,
