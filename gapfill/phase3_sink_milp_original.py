...
def set_lp_solver(solver_name):
    ...
    if solver_name == "highs":
        try:
            ...
            print(
                f"LP solver set to: highs (direct HiGHS with primal simplex - ~4x faster!)"
            )
            return True
        except ImportError:
...
def run_test_on_original_component(
    ...
    stagnation_limit=None,  # Stagnation limit for coverage computation (None = adaptive)
    verbose=True,
    ...
    # Calculate adaptive stagnation limit based on problem characteristics (if not provided)
    ...
    computed_stagnation_limit = stagnation_limit
    if computed_stagnation_limit is None:
        computed_stagnation_limit = max(num_type_a, patience_limit)  # Always test all Type A

    if verbose:
        print(
            f"  Stagnation limit: {computed_stagnation_limit} (Type A: {num_type_a}, patience: {patience_limit})"
        )
    ...
    coverage = compute_coverage_with_temp_sinks(
        ...
        verbose=verbose,
        early_termination=True,
        stagnation_limit=computed_stagnation_limit,
        parallel=parallel,
        n_workers=n_workers,
...
def process_small_component(args):
    ...
    try:
        result = run_test_on_original_component(
            ...
            n_workers=None,
            stagnation_limit=params.get("stagnation_limit"),
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--solver-lp",
        choices=["glpk", "glpk_exact", "scipy", "gurobi", "cplex", "hybrid"],
        default="glpk",
