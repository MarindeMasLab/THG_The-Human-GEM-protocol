...
def test_candidate_batch_parallel(model_json_path, candidate, blocked_rxn_ids, 
                                   temp_sink_mets, deadend_types, n_workers=None, 
                                   zero_cutoff=1e-8, pool=None):
    """
    ...
    if pool is not None:
        ...
        with Pool(processes=n_workers, 
                  ...
                  initargs=(model_json_path, temp_sink_mets, deadend_types)) as pool:
            results = pool.map(_test_single_reaction, args_list)
            for rid, can_carry in results:
                if can_carry:
                    unblocked.add(rid)
...
def compute_coverage_with_temp_sinks(model, candidates, blocked_rxns, deadend_info, 
                                      component_rxn_ids=None, verbose=True,
                                      early_termination=True, stagnation_limit=20,
                                      parallel=False, n_workers=None, model_json_path=None,
                                      solver_lp='glpk'):
    """
    ...
    if parallel:
        if model_json_path is None:
            if verbose:
                print("      Warning: parallel mode requires model_json_path, falling back to sequential")
            parallel = False
        else:
    ...
    if component_rxn_ids:
        ...
        reachability = precompute_candidate_reachability(comp_graph, candidates, blocked_rxns)
        
        # Stats on pre-filtering
        total_pairs = total * n_blocked
        reachable_pairs = sum(len(r) for r in reachability.values())
        if verbose:
            print(f"      Pre-filter: {reachable_pairs}/{total_pairs} pairs reachable ({100*reachable_pairs/total_pairs:.1f}%)")
    else:
        reachability = None
    ...
    for i, cand in enumerate(candidates):
        ...
        if early_termination and cumulative_coverage == blocked_rxns:
            ...
            if verbose:
                print(f"      Early termination: 100% coverage reached after {i} candidates")
        ...
        if stagnation_limit and stagnation_counter >= stagnation_limit:
            ...
            if verbose:
                ...
                print(f"      Stagnation: no new coverage for {stagnation_limit} candidates")
                print(f"      Current coverage: {len(cumulative_coverage)}/{n_blocked} ({pct:.1f}%)")
                print(f"      Skipping remaining {skipped_by_early_term} candidates")
            for j in range(i, total):
                remaining_key = (candidates[j]['met1'], candidates[j]['met2'])
                coverage[remaining_key] = set()
        ...
        if verbose and (i + 1) % 10 == 0:
            ...
            print(f"        {i+1}/{total} ({rate:.2f}/sec, ETA: {eta:.0f}s, coverage: {pct_covered:.1f}%)")
        ...
        if parallel and pool is not None:
            ...
            met1 = cand['met1']
            met2 = cand['met2']
            args_list = [(rid, met1, met2, model.tolerance * 10) for rid in rxns_to_test]
            results = pool.map(_test_single_reaction, args_list)
    ...
    if verbose:
        ...
        if skipped_by_early_term > 0:
            print(f"      Early termination saved: {skipped_by_early_term} candidates not tested")
        pct_covered = 100 * len(cumulative_coverage) / n_blocked if n_blocked > 0 else 0
        print(f"      Final coverage: {len(cumulative_coverage)}/{n_blocked} blocked reactions ({pct_covered:.1f}%)")
...
def solve_greedy(coverage, blocked_rxns, candidates, verbose=True):
    ...
    while uncovered:
        ...
        if verbose:
            ...
            print(f"      Greedy: selected {best_cand['met1']}<->{best_cand['met2']} "
                  f"(+{len(best_coverage)} rxns, total coverage: {pct:.1f}%)")
    
    if verbose:
        total_covered = len(blocked_rxns) - len(uncovered)
        print(f"      Greedy: {len(selected)} PTRs → {total_covered}/{len(blocked_rxns)} reactions covered")
...
def get_candidates_for_original_component(candidates, component_id, used_candidates=None):
    """
...
def run_test_on_original_component(
    ...
    met_to_comp, rxn_to_comp, comp_sizes = build_original_component_map(unconnected_model)
    ...
    rxns_in_orig_comp = [rid for rid, cid in rxn_to_comp.items() if cid == target_component_id]
    mets_in_orig_comp = [mid for mid, cid in met_to_comp.items() if cid == target_component_id]
    ...
    deadends_in_orig = {m: deadend_info[m] for m in mets_in_orig_comp if m in deadend_info}
    if verbose:
    ...
    if sample_blocked and len(blocked) > sample_blocked:
        ...
        if verbose:
            print(f"  Sampled {len(blocked)} blocked reactions for faster coverage computation")
    
    # Get candidates for this ORIGINAL component
    comp_candidates = get_candidates_for_original_component(candidates, target_component_id)
    ...
    comp_candidates = sorted(comp_candidates, key=lambda c: {'A': 0, 'B': 1, 'C': 2}.get(c.get('type', 'C'), 3))
    ...
    if verbose:
        print(f"  Stagnation limit: {stagnation_limit} (Type A: {num_type_a}, patience: {patience_limit})")
    ...
    coverage = compute_coverage_with_temp_sinks(
        phase2_model, comp_candidates, blocked, deadend_info,
        component_rxn_ids=component_rxn_ids, verbose=verbose, early_termination=True,
        stagnation_limit=stagnation_limit, parallel=parallel, n_workers=n_workers,
        model_json_path=phase2_model_json, solver_lp=solver_lp
    )
    ...
    if verbose:
        ...
        print("RESULTS")
        print(f"{'='*60}")
        print(f"Original Component {target_component_id}: {comp_sizes.get(target_component_id, 0)} nodes")
...
def run_phase3_all_components(
    ...
    solver_lp='glpk',
    sample_blocked=None,
    parallel_fba=False,  # Intra-component FBA parallelization
    n_workers_fba=2,
    parallel_components=True,  # Inter-component parallelization (hybrid)
    n_workers_components=4,
    small_component_threshold=500,  # Components < this processed in parallel
    ...
    Args:
        ...
        n_workers_components: Workers for parallel component processing
        small_component_threshold: Components below this size processed in parallel
        skip_main_component: Skip component 1 (already connected)
    ...
    for cid, size in sorted(comp_sizes.items(), key=lambda x: x[1], reverse=True):
        ...
        if component_ids is not None:
            ...
            # Default filtering: skip main component and tiny ones
            if skip_main_component and cid == 1:
                continue
    ...
    if verbose:
        ...
        if component_ids:
            ...
            print(f"  Skipped: component 1 (main), components < {min_component_size} nodes")
    ...
    small_components = [(cid, size) for cid, size in components_to_process 
                        if size < small_component_threshold]
    large_components = [(cid, size) for cid, size in components_to_process 
                        if size >= small_component_threshold]
    ...
    if small_components and parallel_components and n_workers_components > 1:
        if verbose:
            print(f"\n{'='*60}")
            print(f"PHASE A: Processing {len(small_components)} small components in parallel")
        ...
        small_args = [(cid, size, candidates_csv, phase2_model_json, 
                       unconnected_model_json, params) 
                      for cid, size in small_components]
        
        # Process in parallel
        with ProcessPoolExecutor(max_workers=n_workers_components) as executor:
            futures = {executor.submit(process_small_component, args): args[0] 
            ...
            for future in as_completed(futures):
                ...
                try:
                    ...
                    if verbose:
                        n_sel = result.get('selected', 0) if isinstance(result.get('selected'), int) else len(sel)
                        print(f"  [{completed}/{len(small_components)}] Component {result_cid} "
                              f"({result_size} nodes): {n_sel} PTRs, {blocked} blocked")
                except Exception as e:
                    if verbose:
                        print(f"  [{completed}/{len(small_components)}] Component {cid}: ERROR - {e}")
        ...
        if verbose:
            print(f"\n{'='*60}")
            print(f"PHASE A: Processing {len(small_components)} small components sequentially")
            print(f"{'='*60}")
        ...
        for i, (cid, size) in enumerate(small_components):
            ...
            result = run_test_on_original_component(
                candidates_csv, phase2_model_json, unconnected_model_json,
                target_component_id=cid,
    ...
    if large_components:
        if verbose:
            print(f"\n{'='*60}")
            print(f"PHASE B: Processing {len(large_components)} large components sequentially")
    ...
    if verbose:
        ...
        for cid in sorted(component_results.keys()):
            ...
            if 'error' in r:
                ...
                n_sel = r.get('selected', 0) if isinstance(r.get('selected'), int) else 0
                blocked = r.get('blocked', 0) if isinstance(r.get('blocked'), int) else 0
...
if __name__ == '__main__':
    import argparse
    parser = argparse.ArgumentParser(description='Phase 3: Minimize blocked reactions with PTRs')
    
    # Input files
    parser.add_argument('--candidates', default=None, help='Phase-1 candidates CSV')
    parser.add_argument('--phase2-model', default=None, help='Phase-2 connected model JSON')
    parser.add_argument('--unconnected-model', default=None, help='Original unconnected model JSON')
    ...
    parser.add_argument('--parallel-fba', action='store_true',
                       help='Parallel FBA testing within each component')
    parser.add_argument('--workers-fba', type=int, default=2,
                       help='Workers for FBA testing (default: 2)')
    
    # Inter-component parallelization (hybrid approach)
    parser.add_argument('--parallel-components', action='store_true',
                       help='Parallel processing of small components')
    parser.add_argument('--workers-components', type=int, default=4,
                       help='Workers for component processing (default: 4)')
    parser.add_argument('--small-threshold', type=int, default=500,
    ...
    parser.add_argument('--parallel', action='store_true',
                       help='(Legacy) Same as --parallel-fba')
    parser.add_argument('--workers', type=int, default=None,
                       help='(Legacy) Same as --workers-fba')
    ...
    phase2_json = args.phase2_model or os.path.normpath(os.path.join(
        base_dir, '..', 'models', 'base', 'THG-beta-batch_251106_phase2_minimal_connected.json'))
    unconnected_json = args.unconnected_model or os.path.normpath(os.path.join(
        base_dir, '..', 'models', 'base', 'THG-beta-batch_251106_unconnected.json'))
    
    for f, name in [(cand_csv, 'Candidates'), (phase2_json, 'Phase2 model'), (unconnected_json, 'Unconnected model')]:
