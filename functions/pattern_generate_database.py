...
def DefComp(Comp):
    ...
    if Comp.ID1[0] == "C":
        Kegg = (
            '<rdf:li rdf:resource="https://identifiers.org/kegg.compound/'
            + re.findall("C[0-9]+", Comp.ID1)[0]
            + '"/>\n'
        )
    elif Comp.ID1[0] == "G":
        Kegg = (
            '<rdf:li rdf:resource="https://identifiers.org/kegg.compound/'
            + re.findall("G[0-9]+", Comp.ID1)[0]
            + '"/>\n'
        )
    else:
        Kegg = ""
    ...
    ModelComp = (
        ...
        + '''" compartment="'''
        + Comparment
        + '''" initialConcentration="0" hasOnlySubstanceUnits="false" boundaryCondition="false" constant="false" fbc:charge="'''
        + charge
        + '''" fbc:chemicalFormula="'''
        + Formula
        + """">
...
def DefRxn(Rxn, MetEquiv):
    try:
        ...
        corrected_susbtrate = (
            []
        )  # coorective patch to have the rght substrates IDs in RxnList_CL
        for x in Rxn.Substrate:
            ...
            x[2] = (
                x[1].split(":")[-1]
                + "_"
                + str(RxnID.split("_")[1])[0].upper()
                + str(RxnID.split("_")[1])[1:]
            )
            print(x)
            met = x[2].split("_")[0]
            cl = x[2].split("_")[1]
        ...
        ModelRxn = (
            '''      <reaction metaid="'''
            + no
            + '''" sboTerm="SBO:0000176" id="'''
            + no
            + '''" reversible="'''
            + RxnRev
            + '''" fast="false" fbc:lowerFluxBound="'''
            + lb
        ...
        <annotation>
         <rdf:RDF xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#" xmlns:dcterms="http://purl.org/dc/terms/" xmlns:vCard="http://www.w3.org/2001/vcard-rdf/3.0#" xmlns:vCard4="http://www.w3.org/2006/vcard/ns#" xmlns:bqbiol="http://biomodels.net/biology-qualifiers/" xmlns:bqmodel="http://biomodels.net/model-qualifiers/">
            ...
            + """">
              <bqbiol:is>
                <rdf:Bag>
                  <rdf:li rdf:resource="https://identifiers.org/kegg.reaction/"""
            + str(RxnID.split("_")[0])
            + """"/>
                  """
            + RxnEC
...
def DefMod(Gene):
    if Gene.Name():
        ...
        ID = (
            """<rdf:li rdf:resource="https://identifiers.org/hgnc.symbol/"""
            + GeneID
            + """"/>"""
        )
    else:
    ...
    if Gene.Entrez():
        ...
        GeneEntrez = (
            """<rdf:li rdf:resource="https://identifiers.org/ncbigene/"""
            + str(GeneEntrez)
            + """"/>\n"""
        )
    else:
    ...
    if Gene.Ensg():
        ...
        y2 = ""
    ModelMod = (
        '''      <fbc:geneProduct metaid="'''
        + GeneEnsbl
        + '''" sboTerm="SBO:0000243" fbc:id="'''
        + GeneEnsbl
        + '''" fbc:label="'''
        + GeneEnsbl
        + """">
...
def DefGrp(PathNameRxn):
    ...
    for PathName in PathNameRxn:
        groups = ""
        group = (
            '''      <groups:group sboTerm="SBO:0000633" groups:id="group" groups:name="'''
            + PathName
            + """" groups:kind="partonomy">
        <groups:listOfMembers>\n"""
        )
        for RxnID in PathNameRxn[PathName].split():
            idRef = (
                '''          <groups:member groups:idRef="'''
                + str(RxnID.split("_")[0])
                + """"/>\n"""
            )
            groups += idRef
